...
#
dCorIG                     = function(partition_list, w, distance_matrix_list, CClabels){
  ...
  rCClabels = reclassify_cases_gnn(CClabels = CClabels, GNN_Statistic = GNN_Statistic, q_cut_off = 0.80) 
...
# 3. distance_matrix_list = A list of distance matrices.
#
dCorIG_permute          = function(nperm, sample_data, partition_list, w, distance_matrix_list){
  ...
  res = foreach(i = 1:nperm, .export = export.list) %dopar%{
    ...
    dCorIG(partition_list, w, distance_matrix_list, CClabels = pCCLabel )
  }
